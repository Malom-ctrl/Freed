...
window.Freed.Tools = {
    setupSelectionTools: function() {
        ...
        if (clearHighlightBtn) clearHighlightBtn.onclick = this.handleClearHighlight.bind(this);
        ...
        const updateToolbar = () => {
            ...
            if (!selection.rangeCount || selection.isCollapsed || !readerContent.contains(selection.anchorNode)) {
            ...
            for (const h of highlights) {
                if (range.intersectsNode(h)) { hasOverlap = true; break; }
            }
        ...
        readerContent.addEventListener('touchend', () => setTimeout(updateToolbar, 10));
    },
    ...
    handleHighlight: function(e) {
        ...
        const spans = contentEl.querySelectorAll('span[style*="background-color: rgb(255, 255, 0)"], span[style*="background-color: #ffff00"]');
        spans.forEach(s => { s.style.backgroundColor = ''; s.className = 'highlight-outline'; });
    ...
    handleTranslate: async function(e) {
        ...
        try {
            ...
            if (translatedText) {
                ...
                if (span.matches(':hover') && window.Freed.UI && window.Freed.UI.showTooltip) {
    ...
    normalizeHighlights: function(container) {
        let nested = container.querySelectorAll('.highlight-outline .highlight-outline');
        while (nested.length > 0) {
            ...
            nested = container.querySelectorAll('.highlight-outline .highlight-outline');
        }
        let highlights = Array.from(container.querySelectorAll('.highlight-outline'));
        let didMerge = true;
        while(didMerge) {
            didMerge = false;
            ...
            for (let i = 0; i < highlights.length; i++) {
                ...
                if (next && next.nodeType === 1 && next.classList.contains('highlight-outline')) {
