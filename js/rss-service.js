...
(function() {
    ...
    async function fetchAndParseFeed(feed) {
        ...
        try {
            ...
            if (articles.length === 0 && window.Freed.AI) {
                ...
                parsingRule = await window.Freed.AI.generateParsingRule(rawText, feed.url);
    ...
    function parseRSS(xml, feed) {
        ...
        return items.map(item => {
            ...
            let guid = link;
            if(!guid) guid = item.querySelector('guid, id')?.textContent || (title + pubDate);
    ...
    function parseWebWithRule(html, rule, baseUrl, feed) {
        ...
        return items.map(item => {
            try {
                ...
                const snippetEl = rule.snippet ? item.querySelector(rule.snippet) : null;
                const imgEl = rule.image ? item.querySelector(rule.image) : null;

                const title = titleEl ? titleEl.textContent.trim() : 'No Title';
                const link = linkEl ? (linkEl.href || linkEl.getAttribute('href')) : baseUrl;
                ...
                const snippet = snippetEl ? snippetEl.textContent.trim().substring(0, 150) + '...' : '';
    ...
    window.Freed.Service = { fetchAndParseFeed, fetchFullArticle, processAutofetch };
})();