...
window.Freed.Reader = {
    openArticle: async function(articleInput, onRefreshNeeded) {
        ...
        history.pushState({ readingView: true, articleGuid: article.guid }, '', '#article');
        ...
        if (dateEl) {
            try {
                dateEl.textContent = new Date(article.pubDate).toLocaleDateString(undefined, {
        ...
        if (contentEl) {
            ...
            if (article.fullContent) {
                ...
                const baseContent = article.content || article.description || `<p>${article.snippet}</p>`;
